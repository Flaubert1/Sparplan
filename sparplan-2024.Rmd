---
title: "Sparplan der Kids 2024"
date: "Letztes Update: 02.01.2025" #`r format(Sys.time(), '%d.%m.%Y')`
editor_options: 
...
Die Portfolios haben sich gut entwickelt und stehen Ende 2024 deutlich im Plus. Emils Portfolio steht 62,4 % im Plus und Alvas 52,2 %.
```{r}
...
Sparplan_Doku_Emil <- Sparplan_Doku_Emil %>%
  ...
  filter(Closing >= "2020-01-01" & Closing <= "2025-01-01")
...
ggplot(Sparplan_Doku_Emil, aes(x = Closing)) +
  ...
  labs(title = "Portfolio Emil 2020-2024",
       x = "",
       y = "",
       fill = "") +
...
Sparplan_Doku_Alva <- Sparplan_Doku_Alva %>%
  ...
  filter(Closing >= "2020-05-01" & Closing <= "2025-01-01") 
```
...
Sparplan_Doku_Alva %>%
  ...
  labs(title = "Portfolio Alva 2020-2024",
       x = "",
       y = "",
       fill = "") +
...
Der Sparbetrag wird monatlich in den [Nasdaq-100-ETF von Amundi](https://www.justetf.com/en/etf-profile.html?isin=LU1681038243) angelegt. Im Jahr 2024 hat sich der ETF extrem gut entwickelt.
...
ggplot(data = ndx_amundi_2023, aes(x = date, y = adj_perc))+
  ...
  theme_classic()+
  labs(x="", y="Performance", title = "Amundi Nasdaq-100 ETF (A2H577) im Jahr 2024")
...
Das Verhältnis vom Dollar zum Euro hat sich im Jahr 2024 deutlich verändert. Der Euro hat  im Vergleich zum Dollar an Wert verloren. Das ist gut für den Bestand und schlecht für das Kaufen von Anteilen. 
