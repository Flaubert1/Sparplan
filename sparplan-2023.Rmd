...
ggplot(Sparplan_Doku_Emil, aes(x = Closing)) +
  ...
  geom_text(
    aes(y = Gesamt, label = paste0(Gesamt, " €\n", Prozent, "%")),
    hjust = 0.5,
    vjust = -0.5,
    alpha = .8,
    size = 3,
  ...
  geom_point(aes(y = Gesamt), alpha = .5, size = 1) +
  
  geom_point(aes(y = Ansparen), alpha = .5, size = 1) +
  labs(title = "Portfolio Emil im 2020-2023",
       x = "",
       y = "",
       fill = "") +
  scale_y_continuous(limits = c(0, max(Sparplan_Doku_Emil$Gesamt) + 2000),
...
ggplot(aes(x=Closing))+
  ...
  geom_text(aes(y=ETFs, label=paste0(ETFs," €\n ", Prozent, "%")),hjust=0.5, vjust=-0.5, alpha=.8, size = 3, check_overlap = TRUE)+
  geom_point(aes(y=ETFs),alpha=.5, size = 1)+
  geom_point(aes(y=Ansparen),alpha=.3, size = 1)+
 # geom_text(aes(y=Ansparen, label=Ansparen),hjust=0.5, vjust=2, alpha=.8, size = 3)+
  labs(title="Portfolio Alva 2020-2023", x="", y="", fill="")+
  scale_y_continuous(limits = c(0, 15000), labels = dollar_format(suffix = " €", prefix = ""))+
  scale_x_datetime(expand = c(0.05,0),labels = date_format("%b %Y"), breaks = "3 month")+
  theme_classic()+
  ...
  annotate("curve", x = as.POSIXct("2021-05-01"), y = 11000, xend = as.POSIXct("2021-11-01"), yend = 8000, curvature = 0.2, arrow = arrow(length = unit(0.3, "cm"))) +
  # Add text above arrow "Einmalzahlung Schwedenhaus"
  annotate("text", x = as.POSIXct("2021-05-01"), y = 11500, label = "Einmalzahlung\nSchwedenhaus", hjust = 0.5, vjust = 0, size = 3)
...
Der wachstumsorientierte NASDAQ-100-ETF hat sich 2023 sehr gut entwickelt und somit hat sich die etwas riskioreichere Anlagestrategie ausgezahlt. Wie sich die Märkte im Jahr 2024 entwickeln werden, ist ungewiss. Die Analysten sagen, dass die historische Vergleich darauf hindeuten, dass es auch 2024 weiter nach oben geht. Da der Anlagehorizont der Kinder noch sehr lang ist, ist das jedoch unerheblich. Die Hauptsache ist, dass weiterhin kontinuierlich investiert wird.
